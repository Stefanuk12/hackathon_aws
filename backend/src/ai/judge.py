...
Draw rounds are judged on images (with AI reference drawings); Learn and Quick Wit rounds on text.
"""
...
TEXT_TEMPLATES = {m: (PROMPTS / f"judge_{m}.txt").read_text() for m in ("learn", "wit")}
...
def judge_text(mode, prompt, answers):
    """Learn / Quick Wit rounds. answers: list of strings.

    Returns [{answer, rank, score, roast}] where answer is 1-based.
    """
    ...
    return _parse(reply_text(resp), len(answers), key="answer")
...
def _handle_text_round(mode, meta, entries, names, round_no):
    """Learn / Quick Wit: entries carry "text" instead of "s3Key", and there are no reference images."""
